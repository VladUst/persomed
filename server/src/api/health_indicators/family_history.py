...
async def get_all_family_history_info(patient_id: int, db: AsyncSession = Depends(get_async_db)):
    """
...
async def get_family_history_info(patient_id: int, id: int, db: AsyncSession = Depends(get_async_db)):
    repository = FamilyHistoryInfoRepository(db)
    item = await repository.get_by_id(id)
    if not item or item.patient_id != patient_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Запись о семейном анамнезе с ID {id} не найдена")
    return item
...
async def update_family_history_info(patient_id: int, id: int, data: FamilyHistoryInfoCreate, db: AsyncSession = Depends(get_async_db)):
    repository = FamilyHistoryInfoRepository(db)
    item = await repository.get_by_id(id)
    if not item or item.patient_id != patient_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Запись о семейном анамнезе с ID {id} не найдена")
...
async def delete_family_history_info(patient_id: int, id: int, db: AsyncSession = Depends(get_async_db)):
    repository = FamilyHistoryInfoRepository(db)
    item = await repository.get_by_id(id)
    if not item or item.patient_id != patient_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Запись о семейном анамнезе с ID {id} не найдена")
